...
Hall.prototype.setNumberOfColumns = function(_ncols, _gender){
	...
	//RESET POSITIONS!!: Default behaviour whenever the number of columns is changed: RESET to "" all the hall_position
	...
	for(var i in v){ //and re-assign positions
			v[i].hall_position = hallLabels[i]; //by definition hallLabels.length <= v.length
	}
...
/** Whenever the user changes the number of columns, we will try to keep students seated on their place as much as possible
*/
/**Comentado por que este codigo intentaba preservar el orden al cambiar de numero de columnas, pero el asunto, además de 
*complicado, es irrelevante, pues no se espera que un AT este cambiando de numero de columnas. Asi que se vuelve al modelo anterior
* en el que simplemente se resetea el hall al cambiar de numero de columnas 
Hall.prototype.setNumberOfColumns = function(_ncols, _gender){
	this[_gender].ncols = _ncols;
	//When resizing, in order to try to mantain students layout as much as possible, choose the higest vertical dimension based on students positions
	var hr = this.getHighestRow(_gender);
	this[_gender].nrows = Math.max(hr, Math.ceil(this[_gender].num_std / _ncols));
	...
	// primero recorria los estudiantes y cotejaba contra las etiquetas. Como el orden de v no tiene que ver con 
	// la disposicion fisica en la pantalla, no se mantenia el orden. El AT puede haber colocado un estudiante W con menos cursos en la 
	// posición "A1", mientras que en el array (ordernado por cursos) aparece mucho mas atrás, arruinando el intento de mantener el diseño del mapa 
	// al cambiar de numero de columnas
	
	// for(var i in v){ //and re-assign positions
			// var label = v[i].hall_position;
			// var x = hallLabels.indexOf(label);//lineal access is ok
			// if(x != -1){ //Previous student position fits into the new hall dimensions, since his position is on the new set of available positions
				// hallLabels.splice(x,1); //remove it from the bag
			// }
			// else{ //student needs to be reallocated
				// v[i].hall_position = hallLabels[i]; //by definition hallLabels.length <= v.length
				// hallLabels.splice(0,1); //remove it from the begining of the bag
			// }
	// }
	
	
	//Ahora lo hacemos al revés: Para cada etiqueta, busco si algun estudiante la tiene. 
	var hl2 = this.getHallLabels(_gender); //no podemos modificar (splice) hallLabels, pero usaremos hl2
	
	for(var x=0; x < hallLabels.length && v.length > 0; x++){
		var label = hallLabels[x];
		var i = 0; 
		var found = false;
		while(i < v.length && !found){
			if(v[i].hall_position === label){//Previous student position fits into the new hall dimensions, since his position is on the new set of available positions (label)
				found = true;
				v.splice(i,1); //estudiante solucionado, lo sacamos
				hl2.splice(hl2.findIndex( function(e,i){ return e === label;}), 1);
			}
			i++;
		}
	}
	//Asignar las etiquetas sobrantes a los estudiantes sobrantes:
	for(i in v){
		v[i].hall_position = hl2[i];
	}
