...
*/
Layout.prototype.setNumberOfColumns = function(_ncols, callback){ //callback unblocks VIEW-waiting-spinner
	this.hall.setNumberOfColumns(_ncols, this.gender);
	this.render();
	this.save_to_CALM(callback);
}
...
/** For TESTING porpouses. Sets all 'hall_position'  to ""
*/
Layout.prototype.reset_all_positions = function(){
	var v = this.hall.json.sitting.male.old.concat(this.hall.json.sitting.male.new , this.hall.json.sitting.female.old, this.hall.json.sitting.female.new);
	
	var json = { "hall_positions" : {} };
	for(var i = 0; i < v.length; i++){
		json.hall_positions[v[i].id] = "";
	}
	this.save_to_CALM(function(){ console.log("all positions reseted for debugging porpouses");}, json);

}
/** Sends all the students id & hall_position to CALM
...
*/
Layout.prototype.save_to_CALM = function(callback, alternative_content) {
	...
	//Para grabar los datos, CALM espera recibir de SPG las hall positions, y las columnas y filas en un hash con este formato:
	//{ "male": {"number_of_columns": 7, "number_of_rows": 3}, "female": {"number_of_columns": 7, "number_of_rows": 2}. 
	savedContent["male"] = {"number_of_columns": this.hall["male"].ncols, "number_of_rows": this.hall["male"].nrows};
	savedContent["female"] = {"number_of_columns": this.hall["female"].ncols, "number_of_rows": this.hall["female"].nrows};
	...
	var debug = "TRY SAVE: " + JSON.stringify(savedContent);
	$.ajax({
			type: "POST",
            url:  course_url,
            data: savedContent,
			...
			success: function(data)
			{
				...
				console.log(debug + " layout.js SUCCESS: "  + data.updated); // show response from the php script.
				callback(); //related with UI updating, to avoid writing here code belonging to the VIEW
			}
	});
}
...
var draggedElementPosition = undefined;
	...
	rd.event.changed = function (currentCell){
		var std_up = _this.findStudentById(rd.obj.id);
		var std_below = _this.findStudentById(currentCell.getElementsByTagName("div")[0].id);
		...
		console.log("rd.event.changed: " + std_up.display_id + " (cell " + rd.getPosition(rd.obj.id) + ") is flying on top of " + std_below.display_id + " (cell " + rd.getPosition(currentCell.getElementsByTagName("div")[0].id) + "). Updated GLOBAL vars: draggedElement=" + draggedElement.getElementsByTagName("div")[0].id + ", draggedElementPosition=" +draggedElementPosition ); 
