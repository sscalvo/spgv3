...
*/
function Layout(_hall, _gender, _idTable, _ep){ 
	this.hall          = _hall;
	...
	this.male          = { 
							"ncols": _hall["male"].ncols,
							"nrows": _hall["male"].nrows
						  };
    this.female         = { 
							"ncols": _hall["female"].ncols,
							"nrows": _hall["female"].nrows
						  };
...
//	REDIPS.drag.loadContent(id_table_students, "[[\"d16\", 0, 1, \"\", \"C1\"], [\"d17\", 0, 3, \"\", \"C2\"]]");
	...
	//Calculate the maximum rendered height of a cell of this.idTable
	var rowHeights = getRowHeights(this.idTable);
	var lettersTable = this.getLettersTable(rowHeights);
...
Layout.prototype.getContent = function(){ 
	...
	var all_labels = this.hall.getHallLabels(); // male and female
	...
	for(var i=0; i < v.length; i++){ 
		...
		if (label === ""){ //if the student has NO previously asigned seat
			...
			if (pos != -1) {//The student already has a label within the boundaries of the current Hall size..
				...
				content.push([v[i].id, row, col, v[i].old?"old_std":"new_std", this.cellContent(v[i])]); //and add student to the group of people with seats
			}
...
Layout.prototype.cellContent = function(std){
	var cc = "<span class='family_name'><span class='given_name'>" + std.applicant_given_name + " </span>";
	cc += " " + std.applicant_family_name + "</span>";
	cc += "<span class='age'>"+ std.age + "</span><br>";
	//cc += std.display_id + "\n";
	//cc += " id: " + std.id + "\n";
	if(std.old){
		cc += "<div class='big_sitserved'><div class='sitserved'>" + std.courses_sat + "/" + std.courses_served + "</div></div>";
	}
	return cc;
}
