...
Layout.prototype.reset_all_positions = function(new_cols, callback){
	...
	//this.hall.setNumberOfColumns(new_cols, gender);
...
/*			
	rd.event.changed = function (currentCell){ //currentCell: cell we are flying over (not the source cell, but the *maybe* target)
		var msg = ">event.changed\t"
		if(draggedElement == undefined){
			msg += "draggedElement is UNDEFINED\r\n\t";
			draggedElement = rd.obj.id;
			draggedElementPosition = rd.getPosition(rd.obj.id);
		}
		msg += "draggedElement id " + rd.obj.id + " in position " + rd.getPosition(rd.obj.id) + "\r\n\t";
		...
		if(currentCell.getElementsByTagName("div").length != 0){ //Not an empty cell but a cell with an student on it
			msg += "currentCell id " + currentCell.getElementsByTagName("div")[0].id +  " in position " + rd.getPosition(currentCell.getElementsByTagName("div")[0].id) + "\r\n\t";
			std_below = _this.findStudentById(currentCell.getElementsByTagName("div")[0].id);
			msg += "rd.event.changed: " + std_above.display_id + " (cell " + rd.getPosition(rd.obj.id) + ") is flying on top of " + std_below.display_id + " (cell " + rd.getPosition(currentCell.getElementsByTagName("div")[0].id) + "). Updated GLOBAL vars: draggedElement=" + draggedElement.getElementsByTagName("div")[0].id + ", draggedElementPosition=" +draggedElementPosition + "\r\n\t"; 
		}else{
			msg += "currentCell is empty\r\t";
			msg += "rd.event.changed: " + std_above.display_id + " (cell " + rd.getPosition(rd.obj.id) + ") is flying on top of an empty cell (" + rd.getPosition(currentCell.getElementsByTagName("div")[0].id) + "). Updated GLOBAL vars: draggedElement=" + draggedElement.getElementsByTagName("div")[0].id + ", draggedElementPosition=" +draggedElementPosition + "\r\n\t"; 
		}
