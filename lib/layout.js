...
Layout.prototype.getHallLabels = function(){
	...
	var male_labels = [];
	for(var r=65; r<(this.hall.male.nrows+65); r++){
		for(var c=1; c <= this.hall.male.ncols;c++){ 
			male_labels.push(String.fromCharCode(r) + c);
		}
	}

	var female_labels = [];
	for(var r=65; r<(this.hall.female.nrows+65); r++){
		for(var c=1; c <= this.hall.female.ncols;c++){ 
			female_labels.push(String.fromCharCode(r) + c);
		}
	}
	//Females need to be Mirror flip!
	female_labels = reverse_array_in_chunks(female_labels, this.hall.female.ncols);
...
Layout.prototype.getContent = function(){ 
	...
	var labels = this.hall.getHallLabels(this.gender); // male and female
